...
function Quiz() {
    ...
    if (answers.goals === 'optimize') {
        const msg = 'You want to optimize what you already have — start with low-cost swaps: LED bulbs, smart power strips, and better thermostat schedules.'
        console.log("Adding rec:", msg)
        recs.push(msg)
    } else if (answers.goals === 'discover') {
        const msg = 'You want to discover new solutions — consider Energy Star appliances, low-flow fixtures, or even a smart thermostat.'
        console.log("Adding rec:", msg)
        recs.push(msg)
    }
    ...
    if (answers.lighting !== 'led' && answers.bulbCount) {
    ...
    if (answers.phantom === 'always' || answers.smartPowerStrips === 'no') {
        const msg = 'You leave appliances plugged in often — adding smart power strips could reduce phantom loads by $30–$100/year.'
        console.log("Adding rec:", msg)
        recs.push(msg)
    }
    ...
    if (answers.ovenMinutesPerWeek && Number(answers.ovenMinutesPerWeek) > 60) {
        const msg = 'You use your oven a lot — try a toaster oven for small tasks or models with better insulation.'
        console.log("Adding rec:", msg)
        recs.push(msg)
    }
    ...
    if (answers.washerFrequency === '4+') {
        const msg = 'You run laundry/dishwasher 4+ times a week — batching to 1–2 fuller loads per week can lower water and energy use.'
        console.log("Adding rec:", msg)
        recs.push(msg)
    }
    ...
    if (answers.heatingType) {
        const msg = 'Consider heating efficiency: heat pumps are higher efficiency; electric resistance and older gas systems can have higher ongoing costs.'
        console.log("Adding rec:", msg)
        recs.push(msg)
    }
    ...
    if (answers.showerMinutes && answers.lowFlow === 'no') {
        const msg = 'A low-flow showerhead (2.0 → 1.5 GPM) could save ~2,700 gallons/year + $25–$50 in hot water heating.'
        console.log("Adding rec:", msg)
        recs.push(msg)
    }
    ...
    if (answers.ecoConscious === 'rarely') {
        const msg = 'Start with 1–2 easy wins: LED bulbs, shorter showers, and unplugging devices — fast payback.'
        console.log("Adding rec:", msg)
        recs.push(msg)
    }
    ...
    <div style={{ maxWidth: '800px', margin: '6rem auto 3rem', padding: '1rem' }}>
        <h1 style={{ marginBottom: '1rem' }}>WattWise Home Efficiency Quiz</h1>
